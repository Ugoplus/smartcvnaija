  ...
  async processIntent(identifier, intent) {
    switch (intent.action) {
      case 'search_jobs':
        ...
        if (cached) {
          await redis.set(`last_jobs:${identifier}`, JSON.stringify(JSON.parse(cached).rows), 'EX', 3600);
          return this.sendMessage(identifier, JSON.parse(cached).response);
        }
      ...
      case 'apply_job':
        ...
        if (paymentStatus !== 'completed') {
          ...
          const jobIds = intent.applyAll ? JSON.parse(await redis.get(`last_jobs:${identifier}`))?.map(job => job.id) : [intent.jobId];
